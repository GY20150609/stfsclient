"""
Helper script to parse TensorProtos, adapted from TensorFlow.
Link: https://github.com/tensorflow/tensorflow/blob/master/tensorflow/python/framework/tensor_util.py
"""
import numpy as np

from stfsclient.tensorflow import tensor_pb2
from stfsclient.tf_utils import dtypes
from stfsclient.tf_utils import tensor_shape
...
def make_tensor_proto(values, dtype=None):
	"""Expects only np.ndarray or tensor_proto as inputs.
	"""
...
def make_ndarray(tensor):
	"""Create a np.ndarray from a tensor_proto.
	"""
